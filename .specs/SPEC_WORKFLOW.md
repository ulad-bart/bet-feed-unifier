...
- **One folder per problem**: `.specs/<problem-name>/`
    - `<problem-name>` is a short slug describing the problem (e.g., `feed-normalization`, `settlement-validation`).
...
1. Read the existing state of the folder, the relevant code, and `CLAUDE.md`.
2. Identify the decisions it would otherwise make by default.
3. Ask the user **5–7 short clarifying questions**, per `CLAUDE.md` § Clarifying questions.
...
- **Standardized schema changes** — additions/changes to `StandardOddsChangeMessage` / `StandardBetSettlementMessage` or the provider→standard mapping table (see `CLAUDE.md` § Event schema). There is no database and no migrations in this project — do not carry over persistence-layer sections (Flyway, indexes, etc.) unless a real persistence need is introduced.
- **Validation rules** — per field, per provider endpoint.
- **Component integration** — which package(s) under `provider/` (per-provider adapter), `domain/` (standardized model), or `publish/` (outbound port/adapter) are touched, and how the change preserves provider isolation (see `CLAUDE.md` § Architecture and § Project map).
- **CLAUDE.md alignment** — a small table mapping each relevant invariant to how this design honors it.

### `tasks.md`

- Numbered steps, each small enough to land as **one safe commit / one PR** (branch `<problem-name>/<short-name>` — short and descriptive, never the internal `T<step-id>` identifier — per `CLAUDE.md` § PR invariants).
...
- **Per-task execution plan.** Before writing code for a step, the executing agent drafts a plan at `.specs/<problem-name>/plans/T<step-id>-plan.md` (e.g. `.specs/feed-normalization/plans/T02-plan.md` for step 02). The plan covers the concrete edits, file paths, test names, and any open questions surfaced during implementation — it is the working document for that single PR, scoped to one task. `tasks.md` stays the index; the per-task plans hold the detail. This is the spec-flow analog of the repo-root `PLAN.md` convention in `CLAUDE.md` § Working with PLAN.md.
- The agent that executes a step appends a 1–3 line execution result at the end of that step in `tasks.md` (mirrors the `PLAN.md` convention in `CLAUDE.md` § Working with PLAN.md). The corresponding `T<step-id>-plan.md` stays in the repo as history for that PR.
...
- Specs are merged through normal PRs; the build-health and PR invariants in `CLAUDE.md` apply to spec-only changes too (no commented-out blocks, no secrets, etc.).
